...
export default function SkillsSection() {
  ...
  const [quantitySlides, setQuantitySlides] = useState(0);
  const { mode, loading } = useScreenMode();
  const ref = useRef<HTMLDivElement>(null);
  const { width } = useScreenSize();

  useEffect(() => {
    setQuantitySlides(Math.ceil((width - width * 0.05) / 200));
  }, [width]);

  if (!loading) {
    return (
      <div ref={ref} className="overflow-hidden flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-10">
            ...
            >
              ...
              <Carousel
                ...
                wrapperClassName={`${quantitySlides > 11 && "justify-center"}`}
              >
                <JavaScript />
                <Html />
                <Css />
            ...
            >
              ...
              <Carousel
                slidesPerGroup={quantitySlides}
                className={`${colorIcon && "light"} !max-w-full`}
                wrapperClassName={`${quantitySlides > 6 && "justify-center"}`}
              >
                <Linux />
                <Docker />
                <Jest />
